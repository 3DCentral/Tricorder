...
class LcarsAntennaAnalysis(LcarsWidget):
    ...
    # Coordinate helpers
    # ---------------------------------------------------------------
    # Frequency axis is logarithmic 50 MHz .. 2200 MHz (matches the
    # frequency_selector widget for visual consistency).
    ...
    def _freq_to_x(self, freq_hz, rect):
        ...
        if freq_mhz <= 0:
            freq_mhz = self.FREQ_MIN_MHZ
        log_val  = np.log10(freq_mhz)
        log_min  = np.log10(self.FREQ_MIN_MHZ)
        log_max  = np.log10(self.FREQ_MAX_MHZ)
    ...
    def _draw_band_highlights(self, rect):
        ...
        for band in self.known_bands:
            ...
            surf.fill((*band['color'], band['alpha']))
            self.image.blit(surf, (x_start, rect.top))

            # Tiny label at top of band
            lbl = self.font_tiny.render(band['name'], True, band['color'])
    ...
    def _draw_grid(self, rect):
        ...
        # Vertical: log-decade tick marks (same set used by frequency_selector)
