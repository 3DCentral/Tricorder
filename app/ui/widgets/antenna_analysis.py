...
class LcarsAntennaAnalysis(LcarsWidget):
    ...
    def __init__(self, pos, size):
        ...
        # --- NEW: Tuning history (for trend graph) --------------------------
        # Stores dicts: {timestamp, frequency, q_factor, prominence, bandwidth}
        self.tuning_history = []
        ...
        # --- Fonts -----------------------------------------------------------
        try:
            self.font_huge   = pygame.font.Font("assets/swiss911.ttf", 72)  # NEW: For tuning meter
    ...
    def complete_scan(self):
        ...
        # NEW: Add to tuning history if we have a primary resonance in targeted mode
        if self.resonances and self.targeted_mode:
            ...
            if not self.has_baseline:
                ...
                print("Baseline set: {:.2f} MHz".format(primary['frequency'] / 1e6))
    ...
    def _render(self):
        ...
        # NEW: Draw FWHM markers in targeted mode
        ...
        # --- NEW: Draw automatic tuning aids (targeted mode only) -----------
        if self.targeted_mode and self.scan_complete and self.resonances:
            ...
            # Trend graph (bottom-right, if we have history)
    ...
    # ---------------------------------------------------------------
    # NEW: Automatic tuning aid visualizations
    # ---------------------------------------------------------------
    ...
    def _draw_fwhm_markers(self, rect):
        ...
        # Dashed green line at -3dB point
        dash_on, dash_off = 8, 5
        x = rect.left
        while x < rect.right:
            end_x = min(x + dash_on, rect.right)
            pygame.draw.line(self.image, self.color_green, (x, y_half), (end_x, y_half), 1)
            x += dash_on + dash_off
